...
export default function ServiceEditorPage() {
  ...
  const [form, setForm] = useState({
    ...
    metaTitle: "", metaDescription: "", status: "draft", sortOrder: "0",
  });
  ...
  useEffect(() => {
    async function load() {
      try {
        if (!isNew) {
          const item = await api.getServicesById(parseInt(id));
          setForm({
            title: item.title || "", titleNp: item.titleNp || "",
            ...
            status: item.status || "draft", sortOrder: String(item.sortOrder || 0),
          });
        }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isNew) await api.createServices(data);
      else await api.updateServices(parseInt(id), data);
  ...
  return (
    <CMSLayout>
      <div className="mb-6 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold">{isNew ? "New Service" : "Edit Service"}</h1>
